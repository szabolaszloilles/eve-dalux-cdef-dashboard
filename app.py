...
uploaded_sg = st.sidebar.file_uploader(
    "Dalux CDEF-SG export (Excel, optional)",
    type=["xlsx", "xls", "xlsm", "json"], key="sgfile")
...
if uploaded is not None:
    try:
        ...
        _load_err = f"Couldn't read the CDEF export: {e}"

if uploaded_sg is not None:
    try:
        df_sg = load(file_bytes=uploaded_sg.getvalue(), filename=uploaded_sg.name,
                     override_bytes=_ov_bytes)
    except Exception as e:
        _load_err = f"Couldn't read the CDEF-SG export: {e}"

if df is None and df_sg is None:
    import os
...
if df is None and df_sg is None:
    st.info("👈 **Upload your Dalux CDEF export** in the sidebar to load the dashboard. "
            "You can optionally add a **CDEF-SG** export to see it on its own tab.")
    st.caption("Your files are processed in-session and are not stored or shared.")
    st.stop()

# If a combined export was supplied, split SG out automatically
if df is not None and df_sg is None and "is_sg" in df.columns and df["is_sg"].any():
    df_sg = df[df["is_sg"]].copy()
    df = df[~df["is_sg"]].copy()
    if df.empty:
        df = None
...
def render_dashboard(df, _kp, _kp_label, top10_file=None):
    ...
    `_kp`  - key prefix, so widgets in different tabs do not collide
    Returns (fdf, weekly, _rates) for the caller to build the Excel report.
    """
    # ---------------- filters ----------------
    st.sidebar.markdown(f"### Filters — {_kp_label}")
    ...
    sel_disc = st.sidebar.multiselect(
        "Discipline", disc_options, default=disc_options, key=f"{_kp}_disc")
    ...
    if df.empty or valid_dates.empty:
        st.error(
            "No Construction Defect records with a valid *Date created* were found "
            "in that file.\n\n"
            "Check that it is a Dalux **Construction Defect** export (plain or a "
            "sub-type such as *Construction Defect - SG*) and that the "
            "**Date created** column is populated.")
        st.stop()
    ...
    def trend_block(_weekly, title, valcol, dcol, pcol, color, key="", good="down"):
        ...
        with g1:
            ...
            st.plotly_chart(fig, width="stretch", key=f"{_kp}_chart_{key}_{valcol}")
        with g2:
    ...
    if not _perf:
        ...
        st.plotly_chart(figp, width="stretch", key=f"{_kp}_chart_contractor_performance")
...
if top10_file is not None:
    try:
        _t10_x = cd.load_top10_ratios(io.BytesIO(top10_file.getvalue()))
    except Exception:
        _t10_x = None
